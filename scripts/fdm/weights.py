# %% Run this script to compute the weights for the Explicit and Padé schemes
...
a_coefs, b_coefs = fd_pade_weights(
...
a_coefs, b_coefs = fd_pade_weights(m=1, alpha=[0, 1], beta=[0, 1, 2])
a_rationals = [sb.Rational(sb.nsimplify(v)) for v in a_coefs]
b_rationals = [sb.Rational(sb.nsimplify(v)) for v in b_coefs]
sb.pprint(f"α_coefs: {a_rationals}")
sb.pprint(f"β_coefs: {b_rationals}")

a_coefs, b_coefs = fd_pade_weights(m=1, alpha=[-1, 0], beta=[-2, -1, 0])
a_rationals = [sb.Rational(sb.nsimplify(v)) for v in a_coefs]
b_rationals = [sb.Rational(sb.nsimplify(v)) for v in b_coefs]
sb.pprint(f"α_coefs: {a_rationals}")
sb.pprint(f"β_coefs: {b_rationals}")

a_coefs, b_coefs = fd_pade_weights(m=1, alpha=[-1, 0, 1], beta=[-1, 0, 1])
a_rationals = [sb.Rational(sb.nsimplify(v)) for v in a_coefs]
b_rationals = [sb.Rational(sb.nsimplify(v)) for v in b_coefs]
sb.pprint(f"α_coefs: {a_rationals}")
sb.pprint(f"β_coefs: {b_rationals}")
