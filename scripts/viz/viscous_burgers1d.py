...
FIGURES_DIR = Path(__file__).resolve().parent.parent / "figures" / "viscous_burgers1d"
...
TS = "RK1"  # Time-stepping method: "Euler/RK1", "RK2", "RK3", or "RK4"
...
def burgers_rhs(u):
    match SCH:
        ...
        case "PADE":
            return -u * grid.Dx_pade(u) + ν * grid.Dx2_pade(u)
        case "COMPACT":
            return -u * grid.Dx_compact(u) + ν * grid.Dx2_compact(u)
        case "GODUNOV":
            return np.where(
        ...
        case "FLUX_SPLITTING":
            ...
            fP = 0.5 * (f + np.abs(u) * u)
            fN = 0.5 * (f - np.abs(u) * u)
            return -grid.Dx_upwind(fP) - grid.Dx_downwind(fN) + ν * grid.Dx2_central(u)
        case "RUSANOV":
            return -grid.Dx_rusanov(u, lambda u: 0.5 * u * u) + ν * grid.Dx2_central(u)
        case _:
...
fig.tight_layout()
out_snap = FIGURES_DIR / "viscous_burgers1d_snapshots.png"
